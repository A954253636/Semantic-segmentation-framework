...
class transformer_block(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, xmlp  # 输入下一层的x，mlp的x，自注意力后的x
...
class GDCSNet(nn.Module):
    def __init__(self, dim: list = [32, 64, 128, 256, 512], inputchannel: int = 3, classnum: int = 6,
        ...
        self.decoder5 = GatedConvolutions(in_channels=dim[4] * 2, out_channels=dim[3], drop_path=0.)

        self.upsample4 = Upsample(dim[2], dim[2])
        self.decoder4 = GatedConvolutions(in_channels=dim[3] * 2, out_channels=dim[2], drop_path=0.)

        self.upsample3 = Upsample(dim[1], dim[1])
        self.decoder3 = GatedConvolutions(in_channels=dim[2] * 2, out_channels=dim[1], drop_path=0.)

        self.upsample2 = Upsample(dim[0], dim[0])
        self.decoder2 = GatedConvolutions(in_channels=dim[1] * 2, out_channels=dim[0], drop_path=0.)

        self.upsample1 = Upsample(dim[0], dim[0])
        self.decoder1 = GatedConvolutions(in_channels=dim[0] * 2, out_channels=dim[0], drop_path=0.)
...
if __name__ == "__main__":
    ...
    model = GDCSNet([32,64,128,256,512]).to("cuda")

...
class GDCSNet(nn.Module):
    def __init__(self, dim: list = [32, 64, 128, 256, 512], inputchannel: int = 3, classnum: int = 6,
        ...
        self.decoder5 = Conv(in_channels=dim[4] * 2, out_channels=dim[3],kernel_size=1)

        self.upsample4 = Upsample(dim[2], dim[2])
        self.decoder4 = Conv(in_channels=dim[3] * 2, out_channels=dim[2], kernel_size=1)

        self.upsample3 = Upsample(dim[1], dim[1])
        self.decoder3 = Conv(in_channels=dim[2] * 2, out_channels=dim[1], kernel_size=1)

        self.upsample2 = Upsample(dim[0], dim[0])
        self.decoder2 = Conv(in_channels=dim[1] * 2, out_channels=dim[0], kernel_size=1)

        self.upsample1 = Upsample(dim[0], dim[0])
        self.decoder1 = Conv(in_channels=dim[0] * 2, out_channels=dim[0], kernel_size=1)
